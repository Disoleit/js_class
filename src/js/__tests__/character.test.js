...
// Вспомогательная функция для создания персонажа с атакой и защитой
function createCharacter(name, type, attack, defence) {
...
describe('Character Class', () => {
    describe('Constructor validation', () => {
        ...
        test('should create character with attack and defence set after creation', () => {
            const char = createCharacter('Hero', 'Swordsman', 40, 10);
            expect(char.attack).toBe(40);
            expect(char.defence).toBe(10);
        });

        test('should throw error if name is not a string', () => {
            expect(() => new Character(123, 'Bowman')).toThrow('Name must be a string');
            expect(() => new Character(null, 'Bowman')).toThrow('Name must be a string');
            expect(() => new Character(undefined, 'Bowman')).toThrow('Name must be a string');
            expect(() => new Character({}, 'Bowman')).toThrow('Name must be a string');
        });

        test('should throw error if name is too short (less than 2 characters)', () => {
            expect(() => new Character('A', 'Bowman')).toThrow('Name too short (minimum 2 characters)');
            expect(() => new Character('', 'Bowman')).toThrow('Name too short (minimum 2 characters)');
        });

        test('should throw error if name is too long (more than 10 characters)', () => {
            expect(() => new Character('VeryLongName', 'Bowman')).toThrow('Name too long (maximum 10 characters)');
            expect(() => new Character('ThisNameIsWayTooLong', 'Bowman')).toThrow('Name too long (maximum 10 characters)');
        });

        test('should accept name with exactly 2 characters', () => {
            const char = new Character('Jo', 'Bowman');
            expect(char.name).toBe('Jo');
            expect(() => new Character('Jo', 'Bowman')).not.toThrow();
        });

        test('should accept name with exactly 10 characters', () => {
            // Используем строку из 10 символов
            const name10 = 'TenChars10';
            const char = new Character(name10, 'Bowman');
            expect(char.name).toBe(name10);
            expect(() => new Character(name10, 'Bowman')).not.toThrow();
        });

        test('should throw error if type is not in typesList', () => {
            expect(() => new Character('Hero', 'InvalidType')).toThrow('Charecter class not valid');
            expect(() => new Character('Hero', 'Elf')).toThrow('Charecter class not valid');
            expect(() => new Character('Hero', 'Orc')).toThrow('Charecter class not valid');
        });
    ...
    describe('levelUp method', () => {
        test('should increase level by 1', () => {
            const char = createCharacter('Hero', 'Bowman', 25, 25);
    ...
    describe('damage method', () => {
        ...
        test('should calculate damage correctly with 0 defence', () => {
            const char = createCharacter('Hero', 'Swordsman', 40, 0);
            const points = 50;
        ...
        test('should calculate damage correctly with 50 defence', () => {
            const char = createCharacter('Hero', 'Bowman', 25, 50);
            const points = 100;
        ...
        test('should calculate damage correctly with 100 defence', () => {
            const char = createCharacter('Hero', 'Magician', 10, 100);
            char.damage(50);
            expect(char.health).toBe(100);
        });
...
describe('Integration Tests', () => {
    test('should handle battle between different character types', () => {
        const hero = createCharacter('Hero', 'Swordsman', 40, 10);
    ...
    test('should handle battle with level ups', () => {
        const hero = createCharacter('Hero', 'Bowman', 25, 25);
        const enemy = new Zombie('Enemy');
    ...
    test('should handle edge case with multiple characters', () => {
        const characters = [
            createCharacter('Warrior', 'Swordsman', 40, 10),
            new Zombie('Zombie1'),
            new Zombie('Zombie2'),
            createCharacter('Archer', 'Bowman', 25, 25)
        ];
